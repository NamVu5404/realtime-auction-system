...
export const bidApi = {
  ...
  getMyBidHistory: async (
    ...
    try {
      const response = await axiosClient.get<
        ApiResponse<PageResponse<MyBidHistoryResponse>>
      >("/bids/my-history", {
        params: {
  ...
  /**
   * Fetch my bid statistics
   */
  getMyBidStats: async (
    period: "WEEK" | "MONTH" = "MONTH",
  ): Promise<MyBidStatsResponse> => {
    try {
      const response = await axiosClient.get<ApiResponse<MyBidStatsResponse>>(
        "/bids/my-stats",
        {
          params: { period },
        },
  ...
  /**
   * Fetch bid statistics for a specific user (Admin only)
   */
  getBidStatsAdmin: async (
    userId: number,
    period: "WEEK" | "MONTH" = "MONTH",
  ): Promise<MyBidStatsResponse> => {
    try {
      const response = await axiosClient.get<ApiResponse<MyBidStatsResponse>>(
        `/bids/users/${userId}/stats`,
        {
          params: { period },
        },
      );
