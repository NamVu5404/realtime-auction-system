...
public class AiReviewServiceImpl implements AiReviewService {
    ...
    public void review(Auction auction) {
        ...
        String userMessage = "Yêu cầu: Kiểm duyệt nội dung sản phẩm.\n\n<CONTENT>\nTiêu đề: "
                + title + "\nMô tả: " + description + "\n</CONTENT>";
    ...
    @Override
    public PageResponse<AiLogResponse> getAiLogs(Long auctionId, AiReviewDecision decision, Pageable pageable) {
        Page<AiReviewLog> page = aiReviewLogRepository.filterLogs(auctionId, decision, pageable);
        List<AiLogResponse> data = page.getContent().stream()
                .map(log -> AiLogResponse.builder()
                        .id(log.getId())
                        .auctionId(log.getAuction().getId())
                        .auctionTitle(log.getAuction().getTitle())
        ...
        return PageResponse.<AiLogResponse>builder()
    ...
    @Override
    public AiLogCountResponse countAiLogs() {
        List<Object[]> rows = aiReviewLogRepository.countGroupedByDecision();
        long approved = 0;
        long rejected = 0;
        long fallback = 0;
        for (Object[] row : rows) {
            AiReviewDecision d = (AiReviewDecision) row[0];
            long count = (Long) row[1];
            switch (d) {
                case APPROVED -> approved = count;
                case REJECTED -> rejected = count;
                case FALLBACK_TO_ADMIN -> fallback = count;
            }
        }
        return AiLogCountResponse.builder()
                .total(approved + rejected + fallback)
                .approved(approved)
                .rejected(rejected)
                .fallbackToAdmin(fallback)
                .build();
    }

    private AiReviewDecision mapDecision(AiReviewResponse response) {
        AiReviewDecision aiDecision;
        try {
            aiDecision = AiReviewDecision.valueOf(response.getDecision().toUpperCase());
        } catch (IllegalArgumentException | NullPointerException e) {
            log.warn("[AI Review] Unrecognised decision value '{}', falling back to admin", response.getDecision());
            return FALLBACK_TO_ADMIN;
        }

        float confidence = response.getConfidence();
        if (confidence >= APPROVAL_CONFIDENCE_THRESHOLD) {
            return aiDecision == APPROVED ? APPROVED : REJECTED;
        }
